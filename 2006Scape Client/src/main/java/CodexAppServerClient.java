...
public class CodexAppServerClient {
    ...
    private JsonArray dynamicTools() {
        ...
        tools.add(tool("observe_state_if_changed_XS", "Default conditional observation. Use instead of repeated observe_state_XS when polling a stable area; unchanged results return only changed=false, stateHash, critical state, and recent XP.", schema("key", "string", "force", "boolean")));
        tools.add(tool("combat_state_XXS", "Minimal combat check: confirmation plus critical player state and recent XP. Use combat_state_XS when target, loot, or readiness detail matters.", schema()));
        tools.add(tool("combat_state_XS", "Default compact combat observation. Returns HP, food readiness, target NPC, nearby loot, Prayer points/base XP, recent XP gains, and compact combat readiness without a full observe_state payload.", schema()));
        tools.add(tool("observe_state", "Observe current player state, inventory, nearby NPCs, nearby objects, and nearby ground items. Use only when compact XS/XXS observations omit a named field needed for debugging or evidence.", schema()));
        ...
        tools.add(tool("agent_chat_send_XS", "Compact structured chat primitive for agents. Sends a short message to the agent channel, a specific player, or another agent without using public game chat. Use for agent coordination or player-agent messages; pass message plus optional agent/player alias, or to plus toType, plus channel/alsoPublic. Target shortcuts are mutually exclusive: use either agent/player or generic to plus toType, not both. Valid toType values are agent, player, channel, and broadcast. to is required for agent/player targets, and deliverToPlayers is only valid for player/broadcast targets. Direct player delivery may return deliveryPending; if the player is offline when drained, later reads expose undeliveredTo.", schema("message", "string", "text", "string", "agent", "string", "player", "string", "to", "string", "toType", "string", "channel", "string", "alsoPublic", "boolean", "deliverToPlayers", "boolean")));
        tools.add(tool("agent_chat_read_XS", "Read recent structured agent chat messages visible to this claimed profile. Use sinceId from the last read/status call to avoid rereading old messages. Direct player messages include deliveredTo and undeliveredTo arrays.", schema("sinceId", "number", "afterId", "number", "channel", "string", "limit", "number")));
        tools.add(tool("agent_chat_status_XS", "Tiny structured agent chat status: last message id, unread count for a channel, backlog size, and Discord transport status.", schema("sinceId", "number", "afterId", "number", "channel", "string")));
        ...
        tools.add(tool("set_run_XXS", "Toggle run and return only confirmation plus critical player state.", schema("enabled", "boolean")));
        ...
        tools.add(tool("use_inventory_item", "Invoke the normal first-click action on a matching inventory item by name, itemId, itemIds, or slot. Use this for item actions such as herb cleaning that are neither eat/equip/drop nor item-on-item/object.", schema("name", "string", "item", "string", "itemId", "number", "itemIds", "array", "slot", "number")));
        tools.add(tool("click_interface_button", "Dispatch a game interface/action button by buttonId through shared server handlers. Spellbook teleport buttons invoke the authoritative teleport handler and report teleportStarted/teleportStatus; other generic buttons report dispatch separately from verified gameplay completion.", schema("buttonId", "number", "actionButtonId", "number", "id", "number")));
        tools.add(tool("click_interface_button_XXS", "Dispatch an interface button with compact status. Spellbook teleport buttons report whether the teleport actually started; generic button dispatch does not claim gameplay completion.", schema("buttonId", "number", "actionButtonId", "number", "id", "number")));
        tools.add(tool("trade_status_XS", "Compact read-only trade status. Returns trade phase, partner, offered items, confirmations, critical player state, and compact inventory.", schema()));
        tools.add(tool("request_player_trade_XS", "Compact player-trade request primitive. Finds a player by name or playerId, walks closer when just outside the 3-tile trade range, then uses normal trade request/open mechanics; both players must request to open the trade.", schema("name", "string", "player", "string", "targetName", "string", "playerId", "number", "targetPlayerId", "number", "maxDistance", "number", "autoWalk", "boolean", "autoWalkMaxDistance", "number")));
        tools.add(tool("offer_trade_item_XS", "Compact trade-offer primitive. Offers a matching inventory item by name, itemId, itemIds, or slot into the currently open normal trade screen.", schema("name", "string", "item", "string", "itemId", "number", "itemIds", "array", "slot", "number", "amount", "number")));
        tools.add(tool("accept_trade_XS", "Compact safe trade accept primitive. One call validates optional expected partner/item/minAmount, accepts this player's side, records auto-final intent, and completes the normal second confirmation when both parties have each called it once. Each agent controls only its own claimed player; explicit stage/screen is a debug fallback.", schema("stage", "string", "screen", "string", "expectPartner", "string", "expectedPartner", "string", "from", "string", "expectItemId", "number", "expectedItemId", "number", "expectItemIds", "array", "expectItem", "string", "expectedItem", "string", "item", "string", "minAmount", "number")));
        ...
        tools.add(tool("walk_path_steps_XXS", "Queue adjacent route steps and return only confirmation plus critical player state.", schema("steps", "array", "path", "array", "run", "boolean", "allowObjectTransition", "boolean")));
        tools.add(tool("walk_path_steps_XS", "Compact adjacent-step route primitive. Same behavior as walk_path_steps, but returns compact queued-step status and final player/inventory context.", schema("steps", "array", "path", "array", "run", "boolean", "allowObjectTransition", "boolean")));
        ...
        tools.add(tool("walk_to_tile_until_arrived_XXS", "Minimal walk batch: same movement behavior, then confirmation/status plus critical player state only.", schema("x", "number", "y", "number", "height", "number", "stopDistance", "number", "maxTicks", "number", "maxWalkDistance", "number", "stopOnCombat", "boolean", "stopOnStall", "boolean")));
        ...
        tools.add(tool("travel_to_landmark_until_arrived_XXS", "Minimal landmark travel batch: confirmation/status plus critical player state only.", schema("name", "string", "maxTicks", "number", "stopOnCombat", "boolean", "stopOnStall", "boolean")));
        ...
        tools.add(tool("wait_ticks_XXS", "Minimal tick wait: confirmation, waited ticks, critical player state, and recent XP only.", schema("ticks", "number")));
        ...
        tools.add(tool("wait_until_idle_XXS", "Minimal idle wait: completion/status plus critical player state and recent XP only.", schema("maxTicks", "number", "movement", "boolean", "skilling", "boolean", "combat", "boolean", "includeMovement", "boolean", "includeSkilling", "boolean", "includeCombat", "boolean")));
        ...
        tools.add(tool("wait_until_combat_event_XXS", "Minimal combat wait: event/status, critical player state, and XP only. Use XS when loot or target detail matters.", schema("maxTicks", "number", "hpAtOrBelow", "number", "hitpointsAtOrBelow", "number", "lootDistance", "number", "stopOnXpGain", "boolean", "xpGain", "boolean", "stopOnLoot", "boolean", "loot", "boolean", "stopOnTargetDead", "boolean", "targetDead", "boolean", "stopOnCombatEnd", "boolean", "combatEnd", "boolean", "stopOnFoodReady", "boolean", "foodReady", "boolean")));
        tools.add(tool("wait_until_combat_event_XS", "Default compact combat wait. Waits server-side until XP is gained, loot appears, the current target dies, combat ends, HP crosses hpAtOrBelow, food becomes ready, maxTicks is reached, or a blocker occurs. Use instead of repeated wait_ticks during combat.", schema("maxTicks", "number", "hpAtOrBelow", "number", "hitpointsAtOrBelow", "number", "lootDistance", "number", "stopOnXpGain", "boolean", "xpGain", "boolean", "stopOnLoot", "boolean", "loot", "boolean", "stopOnTargetDead", "boolean", "targetDead", "boolean", "stopOnCombatEnd", "boolean", "combatEnd", "boolean", "stopOnFoodReady", "boolean", "foodReady", "boolean")));
        tools.add(tool("wait_until_combat_event_smart_XXS", "Minimal target-aware combat wait. Auto-extends a short maxTicks from the current target's HP, waits briefly for post-death loot, and returns critical state plus event/status only.", schema("maxTicks", "number", "autoExtendWhileSafe", "boolean", "postTargetDeathWaitTicks", "number", "hpAtOrBelow", "number", "hitpointsAtOrBelow", "number", "lootDistance", "number", "freeSlotsAtOrBelow", "number", "stopOnInventoryFull", "boolean", "stopOnXpGain", "boolean", "xpGain", "boolean", "stopOnLoot", "boolean", "loot", "boolean", "stopOnTargetDead", "boolean", "targetDead", "boolean", "stopOnCombatEnd", "boolean", "combatEnd", "boolean", "stopOnFoodReady", "boolean", "foodReady", "boolean")));
        tools.add(tool("wait_until_combat_event_smart_XS", "Default target-aware combat wait. Prefer this for longer fights: it estimates a better maxTicks from the current target, can pause briefly after target death for loot, and returns compact target/loot context.", schema("maxTicks", "number", "autoExtendWhileSafe", "boolean", "postTargetDeathWaitTicks", "number", "hpAtOrBelow", "number", "hitpointsAtOrBelow", "number", "lootDistance", "number", "freeSlotsAtOrBelow", "number", "stopOnInventoryFull", "boolean", "stopOnXpGain", "boolean", "xpGain", "boolean", "stopOnLoot", "boolean", "loot", "boolean", "stopOnTargetDead", "boolean", "targetDead", "boolean", "stopOnCombatEnd", "boolean", "combatEnd", "boolean", "stopOnFoodReady", "boolean", "foodReady", "boolean")));
        ...
        tools.add(tool("attack_npc_XXS", "Attack a live NPC and return only confirmation plus critical player state.", schema("npcIndex", "number")));
        tools.add(tool("cast_spell_on_npc", "Cast a standard combat spell on a live NPC through normal Magic combat mechanics. Prefer this primitive from external Magic-training scripts; pass spellId/magicId from MagicData, spellIndex, or a spell name.", schema("npcIndex", "number", "npcId", "number", "npcIds", "array", "type", "number", "name", "string", "npc", "string", "maxDistance", "number", "reachable", "boolean", "requireReachable", "boolean", "spellId", "number", "magicId", "number", "spellIndex", "number", "spell", "string")));
        tools.add(tool("cast_spell_on_npc_XS", "Compact Magic combat primitive. Casts a standard combat spell on a live NPC and returns spell, NPC, player, inventory, and XP context.", schema("npcIndex", "number", "npcId", "number", "npcIds", "array", "type", "number", "name", "string", "npc", "string", "maxDistance", "number", "reachable", "boolean", "requireReachable", "boolean", "spellId", "number", "magicId", "number", "spellIndex", "number", "spell", "string")));
        tools.add(tool("cast_spell_on_npc_XXS", "Minimal Magic combat primitive. Casts a standard combat spell on a live NPC and returns confirmation plus critical player state and recent XP.", schema("npcIndex", "number", "npcId", "number", "npcIds", "array", "type", "number", "name", "string", "npc", "string", "maxDistance", "number", "reachable", "boolean", "requireReachable", "boolean", "spellId", "number", "magicId", "number", "spellIndex", "number", "spell", "string")));
        ...
        tools.add(tool("set_combat_style_XXS", "Set melee combat XP style and return only confirmation plus critical player state.", schema("style", "string")));
        ...
        tools.add(tool("unequip_items_XXS", "Unequip one or more equipped items and return only confirmation plus critical player state.", schema("equipmentSlot", "number", "equipmentSlots", "array", "slot", "number", "slots", "array", "slotName", "string", "slotNames", "array", "name", "string", "names", "array", "item", "string", "items", "array", "itemId", "number", "itemIds", "array", "all", "boolean")));
        ...
        tools.add(tool("eat_best_food_XXS", "Eat the best food and return confirmation, healed amount, and critical player state.", schema("emergency", "boolean")));
        tools.add(tool("bury_bones", "Bury matching inventory bones by name, itemId, itemIds, or slot using the normal first-click bury mechanics. With no matcher, buries the first inventory bone.", schema("name", "string", "item", "string", "itemId", "number", "itemIds", "array", "slot", "number")));
        tools.add(tool("bury_bones_XXS", "Bury matching bones and return confirmation, buried count, critical player state, and Prayer XP only.", schema("name", "string", "item", "string", "itemId", "number", "itemIds", "array", "slot", "number")));
        tools.add(tool("bury_bones_XS", "Default compact bone-bury tool. Same normal bury mechanics as bury_bones, but returns buried count, inventory delta, Prayer points/base XP, skillChanges, and xpRecent in a compact result.", schema("name", "string", "item", "string", "itemId", "number", "itemIds", "array", "slot", "number")));
        ...
        tools.add(tool("pickup_ground_item_XXS", "Pick up a nearby item and return only confirmation plus critical player state.", schema("name", "string", "item", "string", "itemId", "number", "x", "number", "y", "number", "maxDistance", "number")));
        ...
        tools.add(tool("interact_object_XXS", "Interact with an object and return only confirmation plus critical player state.", schema("objectId", "number", "x", "number", "y", "number", "option", "string", "requireReachable", "boolean")));
        tools.add(tool("interact_object_XS", "Compact object interaction. Same behavior as interact_object, but returns compact object/reachability/walk-target fields and player/inventory context.", schema("objectId", "number", "x", "number", "y", "number", "option", "string", "requireReachable", "boolean")));
        tools.add(tool("object_transition_step_XXS", "Minimal object-transition batch. Interacts with the object, waits until movement/skilling clears, and returns confirmation plus critical state.", schema("objectId", "number", "x", "number", "y", "number", "option", "string", "requireReachable", "boolean", "maxTicks", "number", "combat", "boolean")));
        tools.add(tool("object_transition_step_XS", "Compact object-transition batch for doors, gates, stairs, ladders, and trapdoors. Interacts, waits until idle, and returns compact object/reachability/final-state context.", schema("objectId", "number", "x", "number", "y", "number", "option", "string", "requireReachable", "boolean", "maxTicks", "number", "combat", "boolean")));
        ...
        tools.add(tool("deposit_inventory_items_XXS", "Deposit matching inventory items and return confirmation plus critical player state.", schema("name", "string", "item", "string", "itemId", "number", "itemIds", "array", "keepFoodCount", "number")));
        ...
        tools.add(tool("withdraw_bank_items", "Withdraw matching bank items by name/itemId/itemIds plus amount, or pass items:[{itemId,amount}] for multiple exact item quantities. The player must already be in a bank area.", schema("name", "string", "item", "string", "itemId", "number", "itemIds", "array", "amount", "number", "items", "array")));
        tools.add(tool("withdraw_bank_items_XXS", "Withdraw bank items and return confirmation plus critical player state. Supports legacy itemId/itemIds/name plus shared amount, or compact items:[{itemId,amount}] batches.", schema("name", "string", "item", "string", "itemId", "number", "itemIds", "array", "amount", "number", "items", "array")));
        tools.add(tool("withdraw_bank_items_XS", "Compact bank withdrawal. Supports legacy itemId/itemIds/name plus shared amount, or compact items:[{itemId,amount}] batches such as 9 iron ore and 18 coal; returns per-item withdrawn counts plus compact inventory/bank context.", schema("name", "string", "item", "string", "itemId", "number", "itemIds", "array", "amount", "number", "items", "array")));
        tools.add(tool("bank_item_count_XS", "Compact exact bank-count lookup by itemId, itemIds, name, names, item, or mixed items. Use this instead of full observe_state when the question is how many of specific banked items exist; returns matched ids, canonical names, bank amounts, inventory/equipment amounts, and a concise summary.", schema("itemId", "number", "itemIds", "array", "name", "string", "names", "array", "item", "string", "items", "array")));
        tools.add(tool("food_bank_XXS", "Minimal food/loadout check: critical player state only. Use food_bank_XS when bank or inventory detail matters.", schema()));
        ...
        tools.add(tool("deposit_excess_coins_XXS", "Deposit excess inventory coins and return only confirmation plus critical player state.", schema("keepAmount", "number")));
    ...
    private String userPrompt(String userCommand) {
        return "Player command: " + userCommand + "\n\n"
                ...
                + "Use XXS tools when confirmation plus critical survival state is enough: observe_state_XXS, observe_state_if_changed_XXS, combat_state_XXS, set_run_XXS, walk_path_steps_XXS, walk_to_tile_until_arrived_XXS, travel_to_landmark_until_arrived_XXS, wait_ticks_XXS, wait_until_idle_XXS, wait_until_combat_event_smart_XXS, wait_until_combat_event_XXS, object_transition_step_XXS, interact_object_XXS, click_interface_button_XXS, attack_npc_XXS, eat_best_food_XXS, pickup_ground_item_XXS, bury_bones_XXS, deposit_inventory_items_XXS, withdraw_bank_items_XXS, unequip_items_XXS, and food_bank_XXS. "
                + "Prefer XS tools when the next decision needs inventory, bank, nearby NPC/object/loot, route blocker, equipment, or detailed XP context: observe_state_XS, observe_state_if_changed_XS, combat_state_XS, walk_path_steps_XS, walk_to_tile_until_arrived_XS, wait_until_combat_event_smart_XS, wait_until_combat_event_XS, wait_until_idle_XS, object_transition_step_XS, interact_object_XS, find_nearest_object_XS, bury_bones_XS, deposit_inventory_items_XS, withdraw_bank_items_XS, bank_item_count_XS, agent_chat_read_XS, agent_chat_status_XS, agent_chat_send_XS, unequip_items_XS, and food_bank_XS. Use full observe_state only when XS omits a named field needed for debugging, evidence, or a new workflow. "
                + "Observe first, then use observe_state_if_changed_XS/XXS for stable polling. Enable run with set_run_XXS when energy is available before repeated travel. Use bank_item_count_XS for exact counts of specific bank items, deposit multiple item types in one deposit_inventory_items_XS call with itemIds, withdraw mixed exact quantities in one withdraw_bank_items_XS call with items:[{itemId,amount}], agent_chat_*_XS for concise agent/player coordination with agent/player aliases when targeting a name, and unequip multiple slots/items in one unequip_items_XS call. Compose behavior from stable primitives: use_item_on_item, use_item_on_object, click_interface_button_XXS for confirmation-only button clicks, click_interface_button when UI detail matters, select_interface_item, interact_object_XS, interact_npc, attack_npc, bank/shop tools, wait_until_combat_event_smart_XS, wait_until_idle_XS, and normal movement tools. Keep skill-specific strategy outside Java bridge tools. "
                ...
                + "For combat in an rs-only turn, use combat_state_XS, set_combat_style_XXS, find_training_npc, attack_npc_XXS, wait_until_combat_event_smart_XXS when only HP/XP/event confirmation is needed, wait_until_combat_event_smart_XS when loot or target details matter, eat_best_food_XXS, bury_bones_XS/XXS, and pickup_ground_item_XXS. "
                + "Train attack, strength, and defence toward the requested target by choosing styles explicitly, attacking suitable NPCs, and restocking through normal bank/shop primitives when needed. "
                ...
                + "Pick up useful drops through pickup_ground_item, equip chosen gear with equip_item, and keep death-risk items minimal. "
                + "Avoid dark wizards and other aggressive high-level NPC areas while low level. "
                + "For mining, use find_nearest_rock, interact_object, and wait_until_idle loops, then bank ores when inventory is full. Switch to iron at level 15 if iron is reachable. "
                + "For food, use interact_npc on fishing spots, use_item_on_object plus click_interface_button at ranges/fires, and use_item_on_item with tinderbox/logs for firemaking. "
                + "For shops, travel to a store first, open_nearest_shop, then buy or sell only through shop tools. "
                + "For smithing, use interact_object or use_item_on_object on furnaces, click_interface_button for smelting choices, use_item_on_object with bars on anvils, select_interface_item for item choices, and wait_until_idle for production. "
    ...
    private String developerInstructions() {
        return "You are controlling a 2006Scape player through dynamic tools in namespace rs. "
                ...
                + "Prefer efficient observable loops: observe once, call a server-side batch or idle-wait tool when possible, then use the returned state before deciding the next action. Use observe_state_if_changed_XS/XXS for repeated stable checks, XXS aliases for confirmation-only results, and wait_until_combat_event_smart_XS during combat when loot or target details matter instead of repeated one-tick waits. "
                ...
                + "When a requested gameplay target needs a long grind, continue with efficient primitive loops and preserve useful drops by banking supplies for later account progression. "
                + "For food or skilling blockers, compose NPC/object/item/interface primitives instead of calling skill-specific Java strategy tools. "
