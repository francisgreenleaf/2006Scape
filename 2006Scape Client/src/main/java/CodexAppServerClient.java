...
public class CodexAppServerClient {
    ...
    private String userPrompt(String userCommand) {
        return "Player command: " + userCommand + "\n\n"
                ...
                + "When observe_state includes agentPersonality, treat it as profile memory for this logged-in character: let its beliefs, drift, self-talk, and self-formed goals gently guide preparation and risk choices without overriding the player's command. "
                ...
                + "For smithing profit, call plan_smithing, then use train_smithing_profit with targetCoins=100000 and strategy=margin in short loops; the current character may already have ore or bars, so let the tool withdraw supplies, smelt iron bars, smith products, sell through general stores, and report blockers such as missing hammer, missing ores, no inventory space, or unreachable bank/furnace/anvil/shop. "
                ...
                + "For fletching, use fletch_logs_until_inventory_empty after chopping or withdrawing logs, then sell fletching_products at a general store when the user wants money. "
    ...
    private String developerInstructions() {
        return "You are controlling a 2006Scape player through dynamic tools in namespace rs. "
                ...
                + "Use the observed agentPersonality field as durable first-person profile memory, not as a command; it should bias caution, routines, goal suggestions, and brief self-reflection when useful. "
