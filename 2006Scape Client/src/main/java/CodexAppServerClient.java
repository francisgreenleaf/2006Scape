...
public class CodexAppServerClient {
    ...
    private JsonArray dynamicTools() {
        ...
        tools.add(tool("equip_best_items_XXS", "Equip available combat upgrades and return only confirmation plus critical player state.", schema()));
        tools.add(tool("equip_best_items_XS", "Compact best-gear equip tool. Same behavior as equip_best_items, but returns equipped count plus compact inventory/equipment context.", schema()));
        ...
        tools.add(tool("combat_cleanup_XXS", "Minimal post-kill cleanup loop. Bury bones, pick up useful drops, optionally equip upgrades or set style, and return counters plus critical state.", schema("maxTicks", "number", "maxDistance", "number", "buryBones", "boolean", "pickupDrops", "boolean", "equipUpgrades", "boolean", "style", "string", "itemId", "number", "itemIds", "array")));
        tools.add(tool("combat_cleanup_XS", "Default compact post-kill cleanup loop. Batches bone burying, useful drop pickup, optional gear equip/style setting, and compact loot/inventory/combat context.", schema("maxTicks", "number", "maxDistance", "number", "buryBones", "boolean", "pickupDrops", "boolean", "equipUpgrades", "boolean", "style", "string", "itemId", "number", "itemIds", "array")));
        ...
        tools.add(tool("combat_restock_trip_XXS", "Minimal combat restock batch. Route to a supplied bank target if needed, deposit non-food loot, optionally keep coins, withdraw food, and optionally return; returns compact confirmation.", schema("bankLandmark", "string", "bankX", "number", "bankY", "number", "bankHeight", "number", "bankMaxTicks", "number", "returnLandmark", "string", "returnX", "number", "returnY", "number", "returnHeight", "number", "returnMaxTicks", "number", "foodAmount", "number", "targetFood", "number", "foodItemId", "number", "keepCoins", "number", "coinReserve", "number", "depositNonFood", "boolean", "depositLoot", "boolean", "depositCoins", "boolean")));
        tools.add(tool("combat_restock_trip_XS", "Compact combat restock batch. Routes to bank when a bank target is supplied, batches loot deposit/coin trim/food withdraw, optionally routes back, and returns compact bank/action state.", schema("bankLandmark", "string", "bankX", "number", "bankY", "number", "bankHeight", "number", "bankMaxTicks", "number", "returnLandmark", "string", "returnX", "number", "returnY", "number", "returnHeight", "number", "returnMaxTicks", "number", "foodAmount", "number", "targetFood", "number", "foodItemId", "number", "keepCoins", "number", "coinReserve", "number", "depositNonFood", "boolean", "depositLoot", "boolean", "depositCoins", "boolean")));
    ...
    private String userPrompt(String userCommand) {
        return "Player command: " + userCommand + "\n\n"
                ...
                + "Use XXS tools when confirmation plus critical survival state is enough: observe_state_XXS, observe_state_if_changed_XXS, combat_state_XXS, set_run_XXS, walk_path_steps_XXS, walk_to_tile_until_arrived_XXS, travel_to_landmark_until_arrived_XXS, wait_ticks_XXS, wait_until_idle_XXS, wait_until_combat_event_smart_XXS, wait_until_combat_event_XXS, object_transition_step_XXS, interact_object_XXS, click_interface_button_XXS, attack_npc_XXS, eat_best_food_XXS, pickup_ground_item_XXS, combat_cleanup_XXS, bury_bones_XXS, deposit_inventory_items_XXS, withdraw_bank_items_XXS, unequip_items_XXS, combat_restock_trip_XXS, and food_bank_XXS. "
                + "Prefer XS tools when the next decision needs inventory, bank, nearby NPC/object/loot, route blocker, equipment, or detailed XP context: observe_state_XS, observe_state_if_changed_XS, combat_state_XS, walk_path_steps_XS, walk_to_tile_until_arrived_XS, wait_until_combat_event_smart_XS, wait_until_combat_event_XS, wait_until_idle_XS, object_transition_step_XS, interact_object_XS, find_nearest_object_XS, combat_cleanup_XS, bury_bones_XS, deposit_inventory_items_XS, withdraw_bank_items_XS, unequip_items_XS, combat_restock_trip_XS, and food_bank_XS. Use legacy full tools only when XS omits a field needed for debugging, evidence, or a new workflow. "
                + "Observe first, then use observe_state_if_changed_XS/XXS for stable polling. Enable run with set_run_XXS when energy is available before repeated travel. Deposit multiple item types in one deposit_inventory_items_XS call with itemIds, and unequip multiple slots/items in one unequip_items_XS call. Prefer stable primitives and repo scripts for new behaviors: use_item_on_item, use_item_on_object, click_interface_button_XXS for confirmation-only button clicks, click_interface_button when UI detail matters, select_interface_item, interact_object_XS, interact_npc, attack_npc, bank/shop tools, wait_until_combat_event_smart_XS, wait_until_idle_XS, and route_runner-backed travel. Keep skill-specific strategy in external scripts instead of inventing new Java tools. Legacy batch tools remain available for compatibility. "
                ...
                + "For combat in an rs-only turn, use combat_state_XS, set_combat_style_XXS, find_training_npc, attack_npc_XXS, wait_until_combat_event_smart_XXS when only HP/XP/event confirmation is needed, wait_until_combat_event_smart_XS when loot or target details matter, eat_best_food_XXS, combat_cleanup_XXS after kills, combat_cleanup_XS if loot context matters, and combat_restock_trip_XS/XXS for bank-food refreshes; train_combat and start_combat_goal are compatibility fallbacks. "
