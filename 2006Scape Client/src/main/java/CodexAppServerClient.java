...
public class CodexAppServerClient {
    ...
    private String userPrompt(String userCommand) {
        return "Player command: " + userCommand + "\n\n"
                ...
                + "Observe first. Enable run with set_run when energy is available before repeated travel. Prefer stable primitives and repo scripts for new behaviors: use_item_on_item, use_item_on_object, click_interface_button, select_interface_item, interact_object, interact_npc, attack_npc, bank/shop tools, wait_until_idle, and route_runner-backed travel. Keep skill-specific strategy in external scripts instead of inventing new Java tools. Legacy batch tools remain available for compatibility. "
