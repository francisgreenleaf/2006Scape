...
# Runtime data backup proof should usually come from `scripts/backup-runtime-data.py`; it writes owner-only archive/proof files on POSIX systems, refuses symlinked runtime data and symlinked archive/proof output paths including output parent directories, and readiness validation rejects symlinked proof notes, checks proof/archive owner-only modes where supported, the archive path, `backup archive sha256`, required tar entries for characters/accounts/secrets, the no runtime start/stop/restart proof line, and the `readiness argument: --runtime-data-backup-proof-file ...` line.
# Proof bundles are handoff artifacts only; `package-deployment-proof.py` includes non-secret readiness/proof metadata and excludes runtime backup archives, character saves, account records, secrets, passwords, bridge tokens, and Discord bot tokens.
# Proof templates under `server-deployment/proof-templates/` must be copied and filled; unfilled placeholders are rejected. Prefer `deployment-proof-manifest.json` for final live/manual proof arguments, but store only password env var names, never password values or Discord tokens. Final-gate manifests must keep `require_full_proof:true` in the manifest itself. Use `scripts/check-deployment-proof-manifest.py --check-files` for a quick manifest completeness check that also validates desktop proof evidence and runtime-backup archive/checksum details; `prepare-external-deployment.py --require-full-proof` runs the merged proof check before packaging. This is not a substitute for live readiness proof.
...
scripts/verify-agent-chat-log.py --text-contains MARKER --from-type discord --channel agent
