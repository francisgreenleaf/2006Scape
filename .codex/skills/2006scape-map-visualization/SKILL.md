...
- for active movement maps, keep the visible title short and profile-only, using the selected profile name even when the output file/metadata is Heat Map or fog. Center the profile title horizontally and vertically in the left title-block before the skill tiles. Use the title-bar stats panel for profile skill levels, total level, and playtime instead of explanatory paragraph text. The skill tiles should stay modeled after the client stats tab interface `3917`, using the cache `miscgraphics` tile backing, `staticons`/`staticons2` icons, OSRS yellow text, combat-first menu-style skill order, and the built-in slash divider with same-size current/base level text above-left of the slash and `99` below-right; arrange them as the current wide two-row map strip rather than the exact in-client 3x7 shape.
- for active movement maps, keep the north/top and west/left visual anchor stable, but tighten the east/right and south/bottom bounds after quantization so the map does not carry obvious unused space on those sides.
- keep the footer adventure-ledger stats compact. The current third stats column mixes exact current-save/cache values, such as bank value and damage, with log-derived counters for picked-up loot, buried bones, and estimated kills; do not relabel estimated kills as exact without adding exact kill instrumentation. The footer distance stat is trace-derived, not a map bounding-box span: it sums local filtered movement-edge lengths, assumes one tile is roughly two feet, and displays the result in miles.
