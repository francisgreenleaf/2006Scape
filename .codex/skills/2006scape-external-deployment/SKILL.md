...
The generated `server-deployment/player-handoff-template.md` is the public-safe operator checklist for giving one player access. Use it to send the client zip, transport requirement, private username/password, and optional `/agent` gateway note. Do not send account JSON files, `data/secrets.json`, runtime backup archives, bridge session files, bridge tokens, claim nonces, API keys, or Discord bot tokens.
