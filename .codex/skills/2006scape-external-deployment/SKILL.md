...
The generated `server-deployment/player-handoff-template.md` is the public-safe operator checklist for giving one player access. Use `scripts/render-player-handoff.py --prepared-dir dist/external-deployment --username PLAYER --character CHARACTER --output dist/external-deployment/player-handoff-PLAYER.md` to render a filled player-facing note with checksum, transport, account name, and `/agent` gateway guidance; it never accepts or prints the password. Send the password separately through a private channel. Do not send account JSON files, `data/secrets.json`, runtime backup archives, bridge session files, bridge tokens, claim nonces, API keys, or Discord bot tokens.
