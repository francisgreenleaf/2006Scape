...
It validates the existing screenshot/log evidence file and writes a readiness-compatible `--desktop-client-proof-file` note without starting, stopping, restarting, logging in, or probing runtime.

Use `scripts/deployment-readiness-report.py --require-full-proof` only when all required live/manual proof files and flags exist.
