...
The generated `server-deployment/player-handoff-template.md` is the public-safe operator checklist for giving one player access. For a normal friend-test package with a chosen name, prefer `scripts/prepare-player-package.py PLAYER --character CHARACTER --config "2006Scape Server/ServerConfig.json" --mac-dmg`; it prepares the bundle when needed, creates the ignored PBKDF2 account record, audits it, writes the 20-character generated password only to an owner-only ignored credentials env file, creates the public-safe player kit, and optionally builds a Finder-friendly macOS `.app` plus DMG without printing the password or touching runtime. For a quick VPS/player test when the operator has not picked a name, use `scripts/prepare-player-package.py --random-name --config "2006Scape Server/ServerConfig.json" --mac-dmg --json`; it chooses an unused 1-12 character `Mr...` username/character inspired by existing test names, returns the name plus artifact paths, and writes the password to `privateCredentials`. The agent may read that ignored private env file only when explicitly preparing a private handoff, and must never paste the password into docs, logs, PRs, or public chat. After a prepared bundle exists, the lower-level path is `scripts/provision-player-account.py PLAYER --character CHARACTER --prepared-dir dist/external-deployment`, then `scripts/package-player-kit.py PLAYER --character CHARACTER --prepared-dir dist/external-deployment`, then `scripts/verify-player-kit.py --kit dist/external-deployment/player-kit-PLAYER.zip --prepared-dir dist/external-deployment --username PLAYER --character CHARACTER`. Use `scripts/package-macos-player-app.py PLAYER --character CHARACTER --prepared-dir dist/external-deployment --dmg` to build only the Mac app/DMG wrapper from an existing handoff note. Use `scripts/install-player-account-record.py PLAYER --ssh-target user@example.com --remote-accounts-dir '/opt/2006scape/2006Scape Server/data/accounts'` for a dry-run VPS account-record install plan; add `--apply` only during an intentional deployment. Use `scripts/render-player-handoff.py --prepared-dir dist/external-deployment --username PLAYER --character CHARACTER --output dist/external-deployment/player-handoff-PLAYER.md` when the account/password already exists and only the note needs rendering. Do not send account JSON files, `data/secrets.json`, runtime backup archives, bridge session files, bridge tokens, claim nonces, API keys, or Discord bot tokens.
...
Keep `account_auth_auto_create=false` for external mode. Real public self-signup is future work and should be a separate HTTPS registration service or operator panel with invite codes, rate limits, password-policy enforcement, redacted audit logs, owner-only PBKDF2 account-file creation, and no raw AgentBridge exposure.
