...
For repo Python scripts, expose explicit state intent: `bridge_script.observe_xxs()` for confirmation, `bridge_script.observe_xs()` for decision state, and `bridge_script.observe_full()` for the rare complete payload. Keep legacy `bridge_script.observe()` compatibility calls out of hot loops. If a compact result lacks a field that forces a full observe, either document that call site or add a reusable compact field/primitive so future scripts do not repeat the full read.
