...
Read the returned `status`, `quality`, `safety`, `steps`, `run`, and `runSegments`. ML1 is surface-only for now. If `status=requires-object-transition`, use the object-transition workflow to get through the entrance/ladder/stairs/trapdoor first, then request a new surface route if appropriate. If `status=unsupported-coordinate-layer` or compact output shows non-surface `layers`, do not execute a route; use local path previews, object-transition proof, or a dedicated area runner instead. The old route method is deprecated: do not call bare `route_runner.py --to ...` for normal travel. If live movement is intended, run the returned `cmd`/persisted route-definition path; it uses `execute_route_definition.py` to follow route steps, eat before the next step at `--eat-at 10`, capture nearby NPC context on combat/HP loss, and write route feedback. If movement fails or recovery context is needed, read `route_failure_XS.py` before opening full route evidence JSONL. Use full `route_ml.py define` only when XS omits a field needed for planner debugging.
