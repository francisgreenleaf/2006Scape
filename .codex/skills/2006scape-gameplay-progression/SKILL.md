...
In Python runners, use `bridge_script.observe_xxs()` for confirmation loops and `bridge_script.observe_xs()` for normal decisions. Use `bridge_script.observe_full()` or the legacy `bridge_script.observe()` only for complete bank/equipment/inventory evidence, profile/personality context, or a named missing field. Narrow helpers such as `find_nearest_rock`, `find_nearest_tree`, and object/NPC interactions should not be preceded by a full observe unless the script is proving a specific missing field.
