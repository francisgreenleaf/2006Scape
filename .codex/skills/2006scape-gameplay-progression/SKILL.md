...
- Player-to-player trading: use `request_player_trade_XS`, `offer_trade_item_XS`, `accept_trade_XS`, and `trade_status_XS`. Each profile/agent controls only its own trade window; do not drive the other profile unless the user explicitly asks. `request_player_trade_XS` may auto-walk a short nearby distance into the server's 3-tile trade range, and otherwise returns `tooFar`, `distance`, and `maxDistance`. The offering side opens or answers the request, offers items, then calls `accept_trade_XS` once. The receiving side should call `accept_trade_XS '{"expectPartner":"NAME","expectItemId":995,"minAmount":N}'` or use `receive_trade.py`; the tool validates the offer before accepting, records auto-final intent, and auto-completes the second confirmation when both sides have opted in.
