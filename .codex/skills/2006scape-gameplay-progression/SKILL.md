...
agent-navigation/tools/rs-tool_XS.sh bank_item_count '{"itemIds":[440,453],"names":["coal"]}'
...
In Python runners, use `bridge_script.observe_xxs()` for confirmation loops and `bridge_script.observe_xs()`/`bridge_script.observe()` for normal compact decisions. Use `bridge_script.observe_full()` only for complete bank/equipment/inventory evidence, profile/personality context, or a named missing field. Narrow helpers such as `find_nearest_rock`, `find_nearest_tree`, and object/NPC interactions should not be preceded by a full observe unless the script is proving a specific missing field.
...
- Cowhide combat: if `cowhide_combat_runner.py` is already running, check `python3 agent-navigation/tools/cowhide_combat_runner.py --status` first. To pause it, use `python3 agent-navigation/tools/cowhide_combat_runner.py --request-stop` so it stops at a safe non-combat boundary; if a runner provides `--shutdown-status` or an XS control wrapper, use that for repeated stop polling instead of repeatedly printing verbose `--status`. Do not use process kills unless the runner is wedged and the user has asked for interruption.
- Manual route requests should use ML2 (`agent-navigation/ml2-routing/route_ml_XS.py define`) and the returned `execution.command`. Some existing long runners still route through ML1 internally until migrated; treat those notes as implementation details of the runner, not as the preferred route API for new agent decisions.
- Mining: prefer `python3 agent-navigation/tools/mining_runner.py --target-mining-level N --auto-buy-bronze-pickaxe` for long mining goals because it discovers cache-backed mine sites, chooses live ore targets, routes through ML1 route definitions, mines with compact `find_nearest_rock_XS`/`interact_object_XS`/`wait_until_idle_XS`, and banks ores. Legacy mining tools are compatibility fallback only.
- Woodcutting/fletching: prefer `python3 agent-navigation/tools/fletching_runner.py ...` for chop/fletch loops or `woodcutting_runner.py` for standalone logs. The fletching runners use ML1 route definitions through `bridge_script.route_to`, compact tree interaction through `find_nearest_tree_XS`/`interact_object_XS`/`wait_until_idle_XS`, and primitive fletching on restarted runtimes, with legacy fallback only for old runtimes.
...
- Banking/loadouts: prefer `python3 agent-navigation/tools/bank_loadout.py --preset cowhide-trip --json` for known presets or `bridge_script.execute_bank_policy` inside runners. Do not loop deposits one item type at a time. Use `bank_item_count_XS` when the question is exact counts for specific banked items such as ores, bars, food, or gear; it avoids full `observe_state` and returns matched ids, canonical names, bank amount, inventory amount, and equipment amount. `deposit_inventory_items_XS` accepts `itemIds` for multiple item types in one call and `keepFoodCount` when preserving carried food. `withdraw_bank_items_XS` accepts legacy `itemId`/`itemIds`/`name` plus shared `amount`; for mixed quantities use `items:[{"itemId":440,"amount":9},{"itemId":453,"amount":18}]`. Route-to-bank, loot policy, food target, coin reserve, and optional return routing belong in Python. Use XXS variants only when the counts are already known and confirmation plus critical player state is enough.
...
- Player-to-player trading: use `request_player_trade_XS`, `offer_trade_item_XS`, `accept_trade_XS`, and `trade_status_XS`. Each profile/agent controls only its own trade window. Do not drive, pause, cancel, bank, move, or close interfaces for the other profile merely because it is slow to answer; other players may be in their own productive automation. `request_player_trade_XS` may auto-walk the controlled profile a short nearby distance into the server's 3-tile trade range, and otherwise returns `tooFar`, `distance`, and `maxDistance`. After requesting a trade, wait patiently, polling `trade_status_XS` or compact observe at human-scale intervals for up to the user-provided patience window. If the other player does not respond, report that instead of interrupting them. Only operate the other side when the user explicitly names that profile and asks you to control it, and even then prefer cooperative, non-disruptive trade actions over cancelling unrelated work.
