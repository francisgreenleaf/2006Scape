...
- `agent-navigation/tools/render_profile_map.py`: active selected-profile movement map with cache icons, labels, and run-tinted routes.
...
- `agent-navigation/tools/render_fog_map.py`: active profile fog map with route/icon overlays and a fog-of-war mask.
...
The active movement-map wrappers tighten only the east/right and south/bottom render bounds after quantization, keeping the north/top and west/left anchors stable while avoiding unused space on the right and bottom of the map.

The footer stats include a compact adventure-ledger column. Bank value and damage come from the current profile save plus cache item definitions; loot, bones buried, and estimated kills come from an ignored append-validated local stats cache over agent-session JSONL logs. The distance stat sums local filtered movement-edge lengths from the trace, treating one tile as roughly two feet and converting the total to miles. Treat kills as an estimate unless exact NPC kill counters are added to the save or bridge logs.
...
Default behavior is one worker per active map on a five-minute target cadence. A worker never overlaps the same map; if a render takes longer than five minutes, that map's next pass starts immediately after it finishes. The worker renders to ignored temp files first and atomically replaces only the selected profile's canonical topology PNGs on success; JSON summaries stay under `.local/map-summaries/`. Check progress through `active_map_refresher.py status --profile PROFILE`, `active_map_refresher.py logs --profile PROFILE`, or the profile-scoped status file under `agent-navigation/.local/map-refresh/`. Use `active_map_refresher.py restart --only profile-map --profile PROFILE` for only the profile map, `--trace-profile PROFILE` for profile-specific traces, `--include-unscoped-traces` when legacy unscoped records should remain visible, `--interval-seconds` for a different cadence, and `--serial` to reduce CPU pressure. Use `refresh_active_maps.py --once ...` directly only for one-shot validation or renderer debugging.
