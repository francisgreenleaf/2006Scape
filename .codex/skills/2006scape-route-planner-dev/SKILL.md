...
- `agent-navigation/tools/execute_route_definition.py`: preferred compact ML1 live executor over persisted route definitions. It follows `routeSteps` with bounded lookahead by default (`--lookahead-distance 30`, `--lookahead-step-limit 4`), can preserve old one-step behavior with `--no-lookahead` or `--lookahead-distance 0`, can force/preserve/disable run with `--run-mode`, eats before the next step at `--eat-at` HP, and writes route-batch/outcome evidence under `.local/run-evidence/`.
...
- Long routes should be requested through ML1 (`route_ml_XS.py define`) first. For live movement, run the returned `cmd`/`execution.command` or call `execute_route_definition.py --route-definition PATH`; the executor will walk ahead across bounded route-step batches while still recording route-batch feedback. If a compatibility executor is deliberately used, pass the persisted route definition instead of letting Route Runner re-plan.
- Persisted route-definition JSON currently defaults to the legacy shared `agent-navigation/.local/ml-route-definitions/` directory because `route_ml.py --route-definition-dir` has a concrete default. Profile-scoped run evidence is selected when `--trace-profile` is set, but route-definition artifact paths are shared unless the caller passes `--route-definition-dir`; agent docs should tell runners to use the returned `execution.command` path rather than inventing profile-specific route-definition paths.
...
python3 agent-navigation/ml-routing/route_ml_XS.py route --from PLACE --to PLACE --no-cache-mesh --json
