...
- Coordinate targets such as `--to 3047,3246,0` are valid for surface frontier work; do not create throwaway places just to test a tile.
- ML1 routes only on the level-0 surface map for now: `x=1728..3839`, `y=2560..4031`. Underground-style areas use high Y offsets such as `+6400`, but they are not supported by ML1 until routing becomes entrance-aware. If `define` returns `status=requires-object-transition`, route to the surface-side transition object first and interact with the door/ladder/stairs/trapdoor/entrance. If it returns `status=unsupported-coordinate-layer`, do not execute it; use local path previews, object-transition proof, or a dedicated area runner instead.
