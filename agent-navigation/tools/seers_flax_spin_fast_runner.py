...
PICKABLE_GLOBAL_COOLDOWN_SECONDS = 1.8
PICKABLE_RESPAWN_TICKS = 5
PICK_COOLDOWN_BUFFER_SECONDS = 0.15
...
def choose_cached_flax(player, objects, cooldowns):
    now = time.monotonic()
...
def cooldown_flax(cooldowns, obj, seconds=None):
    key = obj.get("key")
    if not key:
        try:
...
def wait_for_pick_global_cooldown(args, handle, player, last_pick_at):
    if last_pick_at <= 0:
        return player, 0
    ready_at = last_pick_at + PICKABLE_GLOBAL_COOLDOWN_SECONDS + PICK_COOLDOWN_BUFFER_SECONDS
...
def pick_inventory(args, handle, player=None):
    ...
    while int(player.get("freeInventorySlots", player.get("freeSlots", 0)) or 0) > 0:
        ...
        obj = choose_cached_flax(player, known_flax, known_cooldowns)
        source = "cached_flax"
        ...
        if source == "cached_flax" and gained > 0:
            ...
            plant_cooldown_seconds = cooldown_flax(known_cooldowns, obj)
...
def spin_inventory(args, handle, player=None):
    ...
    while count(player, FLAX) > 0 and attempts < args.spin_max_reclicks:
        ...
        wait_started = time.monotonic()
        no_progress = 0
        max_wait_ticks = max(24, attempt_flax * args.spin_max_ticks_per_item)
        attempt_wait_ticks = 0
        chunk_index = 0
        while count(player, FLAX) > 0 and attempt_wait_ticks < max_wait_ticks:
            before_wait_flax = count(player, FLAX)
            before_wait_strings = count(player, BOW_STRING)
...
def build_parser():
    ...
    parser.add_argument("--spin-wait-chunk-ticks", type=int, default=12)
