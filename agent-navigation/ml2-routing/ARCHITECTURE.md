...
This package is the preferred local intelligence layer for new agent-facing A-to-B route requests. It must not mutate game state except through explicit live-control commands. All normal movement still goes through bridge/server mechanics, whether an agent follows ML2 `routeSteps` through movement primitives or deliberately uses a legacy compatibility command. ML1 remains beside it as a fallback and regression-comparison surface while older runners are migrated.
...
- `ml_routing.dataset`: converts existing navigation evidence into stable training/evaluation records and preserves verified object-transition route hints.
- `ml_routing.model`: trains and loads the empirical edge-cost/risk scorer.
- `ml_routing.fast_planner`: answers route requests from the trained model graph without rebuilding all passive traces.
- `ml_routing.transition_catalog`: extracts verified door/gate/stairs/ladder/trapdoor/rope transition metadata from route data.
- `ml_routing.validation`: validates ML2 mixed route definitions and known transition crossings.
...
`ml_routing.collision` mirrors those rules offline. The planner keeps its compact learned graph for speed, but after choosing a macro route it expands route-hint and snap edges into adjacent cache-clipped tiles. It tries each macro waypoint exactly first, then allows a small near-waypoint stop when the saved click target itself is clipped, matching `moveNear=true` walking. This makes map overlays and `recommended.next` follow bridges, docks, fences, walls, and buildings. Object-transition edges are emitted as `type:"object_transition"` route steps rather than pretending a closed door or gate is ordinary walking.
...
`cache_direct` and `cache_mesh` both produce a compact execution shape: `routeSteps` are every-N-tile/turn walk guide rails plus typed object transitions, and `runPlan`/`runSegments` identify hazard-adjacent stretches where a caller should spend run energy instead of walking. `route_ml.py define` wraps those fields in the stable `2006scape.route-definition` API. The ML2 executor treats walk routeSteps as guide rails, walks ahead across bounded chunks, stops before object-transition steps, clicks the object transition primitive, and proves the post side before continuing.
