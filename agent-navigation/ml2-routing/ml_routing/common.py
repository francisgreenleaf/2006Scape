...
def coordinate_layer_transition_block(start: Optional[Tile], target: Optional[Tile]) -> Optional[Dict[str, Any]]:
    ...
    if from_layer == to_layer and from_layer in ("surface", "underground"):
        return None
    ...
    if from_layer != to_layer:
        ...
        message = (
            "ML routing cannot cross surface/underground coordinate layers in one route. "
            "Route to the appropriate entrance, ladder, stairs, trapdoor, or gate first; "
            "use that transition, then request a new route on the destination layer."
        )
    else:
        status = "unsupported-coordinate-layer"
        mode = "unsupported_coordinate_layer"
        reason = "{}_routing_not_supported".format(from_layer)
        message = (
            "ML routing only supports the surface and known underground cache areas. "
            "This coordinate layer is outside the supported route map."
        )
    return {
        "status": status,
        "mode": mode,
