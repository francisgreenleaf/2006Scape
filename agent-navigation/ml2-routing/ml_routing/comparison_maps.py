...
from .benchmark import DEFAULT_CASES, ML2_SHOWCASE_CASES
...
def _ml1_route(args: SimpleNamespace) -> Tuple[Dict[str, Any], float]:
    command = [
        sys.executable,
        ...
        "--from", args.from_tile,
        "--to", args.to,
        "--combat-level", str(args.combat_level),
        "--food", str(args.food),
        "--coins", str(args.coins),
        "--run-energy", str(args.run_energy),
        "--planner", "fast",
        "--json",
        "--graph-snap-distance", str(args.graph_snap_distance),
        "--hazard-buffer", str(args.hazard_buffer),
        "--max-batch-distance", str(args.max_batch_distance),
        "--compress-gap", str(args.compress_gap),
        "--collision-padding-tiles", str(getattr(args, "collision_padding_tiles", 64)),
        "--collision-max-expansions", str(getattr(args, "collision_max_expansions", 250000)),
        "--waypoint-arrival-radius", str(getattr(args, "waypoint_arrival_radius", 1)),
        "--shortcut-max-span", str(getattr(args, "shortcut_max_span", 128)),
        "--shortcut-min-savings", str(getattr(args, "shortcut_min_savings", 4)),
        "--shortcut-corridor-radius", str(getattr(args, "shortcut_corridor_radius", 18)),
        "--route-step-gap", str(getattr(args, "route_step_gap", 10)),
        "--direct-candidate-min-detour", str(getattr(args, "direct_candidate_min_detour", 1.22)),
        "--direct-candidate-min-savings", str(getattr(args, "direct_candidate_min_savings", 24)),
        "--direct-hazard-buffer", str(getattr(args, "direct_hazard_buffer", 10)),
        "--direct-max-expansions", str(getattr(args, "direct_max_expansions", 350000)),
        "--direct-combat-margin", str(getattr(args, "direct_combat_margin", 5)),
        "--runnable-hazard-cost-factor", str(getattr(args, "runnable_hazard_cost_factor", 0.15)),
        "--terminal-hazard-cost-factor", str(getattr(args, "terminal_hazard_cost_factor", 0.25)),
        "--max-warnings", str(args.max_warnings),
        "--max-suspects", str(args.max_suspects),
    ]
    if args.run_enabled:
        command.append("--run-enabled")
    if args.allow_lethal:
        command.append("--allow-lethal")
    if getattr(args, "no_cache_collision", False):
        command.append("--no-cache-collision")
    if getattr(args, "no_shortcut_optimize", False):
        command.append("--no-shortcut-optimize")
    if getattr(args, "no_cache_direct", False):
        command.append("--no-cache-direct")
    if getattr(args, "no_cache_mesh", False):
        command.append("--no-cache-mesh")
    start = time.perf_counter()
    proc = subprocess.run(command, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=Path.cwd())
    elapsed = time.perf_counter() - start
    if proc.returncode not in (0, 2):
        return {
            ...
            "plannerSeconds": round(elapsed, 4),
        }, elapsed
    payload = json.loads(proc.stdout)
    recommended = payload.get("recommended") or {}
    definition = recommended.get("routeDefinition") or {}
    route_steps = recommended.get("routeSteps") or definition.get("routeSteps") or []
    tiles = []
    for step in route_steps:
        tile = _step_completion_tile(step)
        if tile:
            tiles.append(tile)
    result = {
        "planner": "ml1_fast",
        "mode": recommended.get("mode") or definition.get("mode"),
        "status": recommended.get("status") or definition.get("status"),
        "quality": recommended.get("quality") or definition.get("quality"),
        "routeDistance": recommended.get("routeDistance") or definition.get("distanceTiles"),
        "estimatedTicks": recommended.get("estimatedTicks") or definition.get("estimatedTicks"),
        "targetTile": recommended.get("targetTile") or definition.get("targetTile"),
        "arrivalRadius": definition.get("arrivalRadius") or recommended.get("arrivalRadius"),
        "edgeSources": recommended.get("edgeSources") or (definition.get("evidence") or {}).get("edgeSources") or {},
        "routesUsed": recommended.get("routesUsed") or (definition.get("evidence") or {}).get("routesUsed") or {},
        "runPlan": recommended.get("runPlan") or definition.get("runPlan"),
        "runSegments": recommended.get("runSegments") or definition.get("runSegments") or [],
...
def _step_completion_tile(step: Any) -> Optional[Dict[str, int]]:
    if not isinstance(step, dict):
        return None
    if step.get("type") == "object_transition":
        for key in ("postTile", "to"):
            value = step.get(key)
            if isinstance(value, dict) and "x" in value and "y" in value:
                return {"x": int(value["x"]), "y": int(value["y"]), "height": int(value.get("height", 0) or 0)}
    if "x" in step and "y" in step:
        return {"x": int(step["x"]), "y": int(step["y"]), "height": int(step.get("height", 0) or 0)}
    value = step.get("to")
    if isinstance(value, dict) and "x" in value and "y" in value:
        return {"x": int(value["x"]), "y": int(value["y"]), "height": int(value.get("height", 0) or 0)}
    return None
...
def render_case(case: Dict[str, Any], base_args: SimpleNamespace, model: Dict[str, Any], output_dir: Path) -> Dict[str, Any]:
    ...
    include_ml1 = bool(getattr(base_args, "include_ml1_planner", False))
    include_old = bool(getattr(base_args, "include_old_planner", False))
    include_baseline = include_ml1 or include_old
    if include_ml1:
        old, old_seconds = _ml1_route(new_args)
    elif include_old:
    ...
    old_edges = _draw_path(canvas, project, old_tiles, OLD_COLOR, scale, width_factor=1.0) if include_baseline else 0
    new_edges = _draw_path(canvas, project, new_tiles, NEW_COLOR, scale, width_factor=0.55)
    old_run_edges = _draw_run_segments(canvas, project, old_tiles, old.get("runSegments") or [], scale, width_factor=0.55) if include_baseline else 0
    ...
    title_suffix = "ML1 RED  ML2 CYAN  RUN YELLOW" if include_ml1 else ("OLD RED  NEW CYAN  RUN YELLOW" if include_old else "ML2 CYAN  RUN YELLOW")
...
def run_comparison_maps(args: SimpleNamespace) -> Dict[str, Any]:
    ...
    case_set = getattr(args, "case_set", "default")
    if case_set == "showcase":
        cases = ML2_SHOWCASE_CASES
    elif case_set == "all":
        cases = DEFAULT_CASES + ML2_SHOWCASE_CASES
    else:
        cases = DEFAULT_CASES
    if args.case:
