...
from .transition_catalog import transition_from_step
...
def route_hint_edges(generated_at: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    for route in db["routes"]:
        ...
        current = None
        if from_place:
            current = parse_tile(from_place["tile"])
        def add_walk(to_tile: Dict[str, int]) -> None:
            nonlocal current
            if not current:
                current = to_tile
                return
            if navdb.tile_key(current) == navdb.tile_key(to_tile):
                return
            segments.append({
                "type": "walk",
                "fromTile": dict(current),
                "toTile": dict(to_tile),
            })
            current = to_tile
        ...
        for step in route.get("steps", []):
            ...
            if transition:
                pre_tile = parse_tile(transition.get("preTile"))
                post_tile = parse_tile(transition.get("postTile"))
                if pre_tile:
                    add_walk(pre_tile)
                if pre_tile and post_tile:
                    segments.append({
                        "type": "object_transition",
                        "fromTile": dict(pre_tile),
                        "toTile": dict(post_tile),
                        "transition": transition,
                    })
                    current = post_tile
                continue
            tile = parse_tile(step.get("to")) or parse_tile(step.get("near"))
            if tile:
                add_walk(tile)
        if to_place:
            to_tile = parse_tile(to_place["tile"])
            if to_tile:
                add_walk(to_tile)

        for index, segment in enumerate(segments):
            left = segment["fromTile"]
            right = segment["toTile"]
            ...
            records.append({
                ...
                "routeIndex": index,
                "stepType": segment.get("type", "walk"),
                ...
                "objectStepCount": 1 if segment.get("type") == "object_transition" else 0,
                "objectTransition": segment.get("type") == "object_transition",
