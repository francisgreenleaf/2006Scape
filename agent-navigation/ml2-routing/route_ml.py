#!/usr/bin/env python3
"""One-command entrypoint for the isolated 2006Scape ML2 routing pipeline."""
...
DEFAULT_ROUTE_DEFINITION_DIR = "agent-navigation/.local/ml2-route-definitions"
...
def persist_route_definition(args: argparse.Namespace, definition: dict | None) -> dict | None:
    ...
    if configured_dir:
        ...
        safe_profile = "".join(ch for ch in str(profile).lower() if ch.isalnum() or ch in ("-", "_")) or "profile"
        output_dir = Path(DEFAULT_ROUTE_DEFINITION_DIR) / safe_profile
    else:
...
def add_shared_route_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--planner", choices=["fast", "full"], default="fast",
                        help="fast uses the isolated ML2 graph; full wraps router.py/route_eval.py and is slower.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    compare.add_argument("--case-set", choices=["default", "showcase", "all"], default="default",
                         help="default renders original benchmark cases; showcase renders the 10 ML2 capability cases.")
    compare.add_argument("--include-ml1-planner", action="store_true",
                         help="Also run and draw ML1 fast route output in red for ML1-vs-ML2 comparisons.")
