...
For external-player experiments, keep the local dev flow unchanged and start from the explicit sample config. Copy it to the ignored runtime config, replace placeholder hosts such as `REPLACE_WITH_PUBLIC_INTERFACE_IP` and `server.example.com`, then preflight before starting:
