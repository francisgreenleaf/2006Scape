...
- For `client_tls_tunnel`, packaged client `server.host` must be localhost/loopback because the Java client still speaks plaintext to a player-side local tunnel. `public_game_host` is the remote TLS endpoint for deployment checks, not the client jar's direct socket target.
