...
- Only one gameplay controller may own a profile. Start unattended runners through `launch_detached_runner.py --supervise`; it and the ML2 executor share `.local/runners/<profile>/active-controller.json`, and nested ML2 subprocesses inherit the active runner lease. Use `python3 agent-navigation/tools/gameplay_controller_XS.py status --profile PROFILE`, `status --all`, or `stop --profile PROFILE --wait 5`; do not edit lease files or use process-name kills. A standalone ML2 `controller_conflict` is a hard no-action result until the existing controller releases.
...
- Mac player DMGs should feel like normal end-user apps: `scripts/package-macos-player-app.py` creates an iconed `agent-scape.app`, sets the bundle icon, launches through the packaged client with Finder-safe Java path discovery, shows a macOS alert on launch failure, and writes details to `~/Library/Logs/agent-scape/agent-scape-launch.log`. Keep the zip/player-kit flow for Windows/Linux and checksum-first handoffs.
