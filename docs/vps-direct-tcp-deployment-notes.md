...
- Active VPS: `64.225.118.140`.
- SSH from the repo machine: `ssh -i ~/.ssh/2006scape-do-nopass root@64.225.118.140`.
...
- 2026-06-14: Server process listens on public `64.225.118.140:{43594,8080,43595}`, loopback `127.0.0.1:{43594,8080,43595}`, and loopback-only agent bridge `127.0.0.1:43610`.
...
- 2026-06-14: Added VPS PBKDF2 accounts and copied character saves for `MrFlame`, `MrFish`, `MrWood`, and `MrAthlete`. Live protocol login probes accepted all four over `64.225.118.140:43594`.
...
The packaged client connects to `64.225.118.140` on world `1`, game port `43594`, HTTP cache port `8080`, and JAGGRAB port `43595`.
...
```sh
ssh -i ~/.ssh/2006scape-do-nopass -N -L 43610:127.0.0.1:43610 root@64.225.118.140
```
...
source dist/external-deployment/private/vps-character-credentials.env
python3 scripts/probe-game-login.py --host 64.225.118.140 --port 43594 --username "$MRFLAME_USERNAME" --password-env MRFLAME_PASSWORD --hold-seconds 1
```
