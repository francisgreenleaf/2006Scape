...
If the operator wants a quick generated account name, `scripts/prepare-player-package.py --random-name --config "2006Scape Server/ServerConfig.json" --mac-dmg --json` chooses an unused `Mr...` username/character inspired by the existing test accounts, creates the same 20-character generated password used by the named path, and returns artifact paths. The password stays only in the ignored `privateCredentials` env file for private handoff.
